import { getDefaultCatalog, findCatalogEntry, updateCatalogEntry, normalizeText } from './src/config/tankCatalog.js';
...
const testNames = [
  ...
  { input: { nombre: 'Zona Industrial', display_name: 'Zona Industrial', tag: 'NIVEL_DE_ZONA_INDUSTRIAL' }, expectedId: 'zona industrial' },
...
for (const test of testNames) {
  const found = findCatalogEntry(test.input, getDefaultCatalog());
...
for (const { a, b } of collisionTests) {
  const foundA = findCatalogEntry(a, getDefaultCatalog());
  const foundB = findCatalogEntry(b, getDefaultCatalog());
...
for (const entry of getDefaultCatalog()) {
  if (!entry.aliases || entry.aliases.length === 0) continue;
  const alias = entry.aliases[0];
  const found = findCatalogEntry({ nombre: alias, display_name: alias, tag: alias }, getDefaultCatalog());
...
const found = findCatalogEntry(refTank, getDefaultCatalog());
const updated = updateCatalogEntry(getDefaultCatalog(), found.id, {
  area_m2: 999,
  altura_rebose: 12.34,
  altura_total: 15.67,
  volumen: 888,
